...
The Model Context Protocol (MCP) is the foundation of HUD. It provides a standard way for AI agents to interact with any software environment through tool calls.
...
1. **Setup Tools**: Initialize environment state (`setup_board`, `goto_url`)
...
mcp_config = {
  "hud-text-2048": {
    "command": "docker",
    "args": ["run", "--rm", "-i", "hudpython/hud-text-2048:v1.1"]
  }
}
```
