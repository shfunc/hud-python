---
title: "HUD SDK | Docs"
description: "A Python SDK for interacting with HUD environments and evaluation benchmarks for browser use and computer use models."
---

> **Alpha Release Notice**: This SDK is currently in beta status (v0.1.4). The API is still evolving and may change in future releases as we gather feedback and improve functionality.

[Visit our homepage hud.so →](https://hud.so)

## Overview

HUD provides an interface for:

* Connecting to HUD evaluation environments such as OSWorld

* Running benchmarks across various tasks

* Supporting different agent adapters

* Providing an asynchronous API for efficient interactions

## Get Started

Follow our [Installation Guide](/installation) to get set up, then check out the [Quickstart Guide](/quickstart) to run your first example.