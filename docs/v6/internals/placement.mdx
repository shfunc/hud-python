...
`hud eval` chooses differently on purpose: it always starts from a file, so it isolates each
rollout — container rows in Docker, bound rows in a child process serving the env's source
(`SubprocessRuntime`) — instead of serving a live env in-process.
