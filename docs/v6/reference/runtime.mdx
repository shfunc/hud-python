...
Runs an environment in this process. `source` can be:
...
- **a `.py` file or directory** that declares it, imported fresh per rollout (sibling imports
  ...
  task's env.
- **a live `Environment`** - served directly.
...
- **`source`** - `.py` file (or directory) that declares the env, or a live `Environment`, which is served from the file its `@env.template` declarations live in. The child's working directory is the source's directory, so sibling imports and relative data paths resolve.
- **`env`** - pin a specific env name when a source path declares more than one. Defaults to the placed task's env.
