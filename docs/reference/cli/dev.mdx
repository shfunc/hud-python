...
**Live Changes**: Any edits to files in `src/` are immediately reflected in running containers without restart.

**Hot-Reload Supervisor**: Optionally injects a supervisor process that can restart the MCP server when needed (disabled with `--no-reload`).
