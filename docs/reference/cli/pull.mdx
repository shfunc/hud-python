...
<ParamField path="--lock" type="string">
  Path to a lock file to read (alternative to using the image ref). Short: `-l`
  When provided, the command reads metadata from the specified lock file instead of the registry.
</ParamField>
...
Use an existing lock file to pull an environment:

```bash
hud pull ./env.lock.yaml
```
