...
<ParamField path="--platform" type="string" default="linux/amd64">
  Target platform for Docker build. Defaults to RL-compatible `linux/amd64` when not provided.
</ParamField>
...
Force a Linux AMD64 build on non-Linux hosts:
...
2. Auto-generates: `{directory-name}:dev`
3. Uses the directory name as-is (no `hud-` prefix added)

Examples:
- Directory `my_tool` → Image `my_tool:dev`
- Directory `browser-env` → Image `browser-env:dev`
...
<!-- Build arguments are not currently exposed via CLI flags -->
