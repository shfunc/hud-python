...
## 5. Using Tracing with MCP Agents

If you're working with MCP-based agents, you can use the tracing functionality to capture and analyze MCP calls:
...
# Use the trace context manager to capture MCP calls
with hud.trace("example_trace"):
    result = await agent.run(
        "Find information about capybaras",
        max_steps=5,
    )
    print(f"Result: {result}")

# Trace is automatically uploaded and available at the URL shown in the logs
```

You can also use the `@hud.register_trace` decorator to trace entire functions:

```python
@hud.register_trace(name="search_function")
...
For more details on tracing, see the [Tracing documentation](/advanced/tracing).
...
*   Learn about [Tracing](/advanced/tracing) to capture and analyze MCP calls in your applications.