...
# Note: Uses a fork of verifiers for multimodal support
```
...
Training uses YAML configs to map agent tools to MCP tools. The actual config is loaded from the cloned hud_vf_gym:

```yaml
# From hud_vf_gym/verifiers/configs/2048.yaml
system_prompt: |
...
env = vf.load_environment(
    ...
    config_path="verifiers/configs/your-env.yaml",  # In hud_vf_gym
    num_tasks=10
)
