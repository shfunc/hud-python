...
class TodoCustomSeedSetup:
    ...
    async def __call__(self, context, items: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            created_items = []
            for item in items:
                create_result = await context.call_app_api("todo", "/api/items", method="POST", data={
...
class TodoCompositeSetup:
    ...
    async def __call__(self, context, preset: str = "basic", extra_items: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if preset == "basic":
                seed_result = await context.execute_setup({"function": "todo_seed", "args": {"num_items": 3}})
                results.append({"phase": "basic_seed", "result": seed_result})
            elif preset == "full":
                seed_result = await context.execute_setup({"function": "todo_seed", "args": {"num_items": 5}})
