...
class BrowserComputerTool:
    ...
    async def __call__(
        ...
        pattern: list[int] | None = Field(
            None, description="Multi-click pattern (delays in ms)"
        ),
        # Type parameters
    ...
    async def _click(
        ...
        try:
            ...
            return ToolResult(
                output=f"Clicked at ({x}, {y}) in browser",
    ...
    async def _scroll(
        ...
        try:
            ...
            screenshot = await self._get_screenshot_base64()
            return ToolResult(
                output=f"Scrolled by ({delta_x}, {delta_y}) at ({x}, {y})",
                base64_image=screenshot
            )
