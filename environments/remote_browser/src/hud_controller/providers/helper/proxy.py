...
async def _decodo_proxy() -> Optional[Dict[str, Any]]:
    ...
    if not rotating:
        ...
        return {"type": "custom", "server": f"{host}:{port}", "username": user, "password": pwd, "active": True}
...
def _standard_proxy() -> Optional[Dict[str, Any]]:
    ...
    if not server:
        return None
    return {"type": "custom", "server": server, "username": os.getenv("PROXY_USERNAME"), "password": os.getenv("PROXY_PASSWORD"), "active": True}
