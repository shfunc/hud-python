...
class SheetContainsEvaluator(BaseEvaluator):
    ...
    async def __call__(
        self, context: Any, args: Union[str, List[str]], partial_rewarding: bool = False, **kwargs
    ) -> EvaluationResult:
        ...
        if not context or not hasattr(context, "page") or not context.page:
            ...
            return {"reward": 0.0, "done": False, "info": {"error": "No browser page available"}}
        ...
        if isinstance(args, str):
            ...
            return {
                ...
                "done": False,
                "info": {
                    "error": f"Invalid args format. Expected string or list, got {type(args)}"
                },
            }

        if not search_terms:
            logger.error("No search terms provided")
            return {"reward": 0.0, "done": False, "info": {"error": "No search terms provided"}}
