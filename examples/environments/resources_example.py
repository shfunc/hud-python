...
class HudResourcesDemo:
    ...
    async def explore_evaluators_registry(self):
        ...
        try:
            ...
            if evaluators:
                ...
                print(f'       "config": {{"function": "{example_evaluator}", "args": {{"expected_count": 2}}}}')
                print(f'   }})')
    ...
    async def explore_setup_registry(self):
        ...
        try:
            ...
            if setup_tools:
                ...
                print(f'       "config": {{"function": "{example_setup}", "args": {{"num_items": 5}}}}')
                print(f'   }})')
    ...
    async def explore_problems_registry(self):
        ...
        try:
            ...
            if problems:
                ...
                print(f'   await session.connector.call_tool("setup", {{"config": {{"name": "{example_problem}"}}}})')
                print(f'   # Evaluate the problem')
                print(f'   await session.connector.call_tool("evaluate", {{"config": {{"name": "{example_problem}"}}}})')
    ...
    async def demonstrate_practical_usage(self):
        ...
        try:
            ...
            easy_problems = [name for name, info in problems.items() 
                           if info.get("difficulty") == "easy"]
            ...
            setup_result = await self.session.connector.call_tool("setup", {
                "config": {"name": chosen_problem}
            })
            ...
            eval_result = await self.session.connector.call_tool("evaluate", {
                "config": {"name": chosen_problem}
            })
