...
async def main():
    ...
    mcp_config = {
        "local": {
            ...
            "args": [
                ...
                "--rm",
                "-i",
                "-p", "8080:8080",  # noVNC web interface
                "hud-browser"
            ]
        }
    }
    ...
    try:
            ...
            if response.tool_calls:
                ...
                if steps % config["check_interval"] == 0:
                    ...
                    if consecutive_no_progress > 3:
                        print(f"   ⚠️ No progress for {consecutive_no_progress * config['check_interval']} steps")
        ...
        if task.evaluate_tool:
            try:
                final_eval = await client.call_tool("evaluate", {
                    "name": config["evaluate_name"],
                    "arguments": config["evaluate_args"]
                })
            except Exception as e:
