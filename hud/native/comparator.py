...
def _compare_exact(value: Any, reference: Any, **kwargs) -> tuple[bool, float, dict]:
    """Exact comparison."""
    matches = value == reference
    return matches, 1.0 if matches else 0.0, {}


def _compare_fuzzy(value: Any, reference: Any, threshold: float = 0.8, **kwargs) -> tuple[bool, float, dict]:
...
def _compare_numeric(value: Any, reference: Any, tolerance: float = 1e-6, **kwargs) -> tuple[bool, float, dict]:
...
def _compare_semantic(value: Any, reference: Any, threshold: float = 0.8, tolerance: float = 1e-6, **kwargs) -> tuple[bool, float, dict]:
    ...
    if isinstance(v_obj, dict) and isinstance(r_obj, dict):
        ...
        if len(v_obj) == len(r_obj):
            matching = sum(1 for a, b in zip(v_obj, r_obj) if a == b)
...
def auto_select_mode(value_type: DataType, ref_type: DataType) -> ComparisonMode:
    ...
    if value_type in (DataType.INTEGER, DataType.FLOAT) or ref_type in (DataType.INTEGER, DataType.FLOAT):
    ...
    async def __call__(
        ...
        if value is None or reference is None:
            return EvaluationResult(
                ...
                content="Missing value or reference",
                isError=True
            )
        ...
        if isinstance(value, list) and isinstance(reference, list):
            if len(val_list) != len(ref_list):
                return EvaluationResult(
                    ...
                    content=f"Error: Mismatched lengths - {len(val_list)} values vs {len(ref_list)} references"
                )
            comp_type = "batch"
        ...
        # Format content
        prefix = {"scalar": "Single", "batch": "Batch", "broadcast": "Broadcast"}.get(comp_type, "Unknown")
        mode_name = results[0].comparison_mode.value.capitalize()
        
        return EvaluationResult(
            reward=avg_similarity,
            done=all_match,
            content=f"{prefix} {mode_name}: {match_count}/{len(results)} matches, avg={avg_similarity:.3f}"
        )
