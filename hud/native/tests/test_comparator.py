...
from hud.tools.submit import set_submission, get_submission
...
class TestAutoSelectMode:
    ...
    @pytest.mark.parametrize("value_type,ref_type,expected_mode", [
        # JSON gets semantic
...
class TestAliasPreprocessing:
    ...
    async def test_numeric_alias_preprocessing(self):
        ...
        result = await float_tool.fn(
            value='After careful calculation, the answer is 3.14159',
            reference='3.14159'
        )
        assert result.done
        assert result.reward == 1.0
        ...
        result = await int_tool.fn(
            value='The count is exactly 42 items',
            reference='42'
        )
        assert result.done
        assert result.reward == 1.0
    ...
    async def test_boolean_alias_preprocessing(self):
        ...
        result = await bool_tool.fn(
            value='Based on the analysis, the statement is TRUE',
            reference='true'
        )
        assert result.done
        assert result.reward == 1.0
    ...
    async def test_list_alias_preprocessing(self):
        ...
        result = await list_tool.fn(
            value='The sorted results are [1, 2, 3, 4, 5]',
            reference='[1, 2, 3, 4, 5]'
        )
        assert result.done
        assert result.reward == 1.0
    ...
    async def test_complex_llm_output(self):
        ...
        reference = '{"status": "success", "data": {"values": [1, 2, 3], "metadata": {"timestamp": "2024-01-01", "version": "1.0"}}}'
