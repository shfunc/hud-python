...
class MCPClient:
    ...
    async def create_sessions(self) -> dict[str, MCPSession]:
        ...
        if self.verbose and self._sessions:
            for name, session in self._sessions.items():
                logger.debug(f"  - {name}: {type(session).__name__}")
    ...
    async def fetch_telemetry(self) -> dict[str, Any]:
        ...
        for server_name, session in self._sessions.items():
            try:
                ...
                try:
                    result = await session.connector.client_session.read_resource("telemetry://live")
    ...
    async def read_resource(self, uri: str) -> types.ReadResourceResult | None:
        ...
        for server_name, session in self._sessions.items():
            try:
                ...
                if self.verbose:
                    logger.debug(
                        "Successfully read resource '%s' from server '%s'",
                        uri,
                        server_name
                    )
                ...
                if self.verbose:
                    logger.debug(
                        "Could not read resource '%s' from server '%s': %s",
                        uri,
                        server_name,
                        e
                    )
    ...
    def get_sessions(self) -> dict[str, MCPSession]:
