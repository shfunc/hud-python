...
def get_docker_manifest(image: str) -> Optional[dict]:
...
def get_image_size_from_manifest(manifest: dict) -> Optional[int]:
    ...
    try:
        ...
        if "layers" in manifest:
            ...
            # Just take the first one for size estimate
...
def pull_environment(
    ...
    if lock_file or target.endswith('.yaml') or target.endswith('.yml'):
        ...
        if not image_ref:
            design.error("No image reference found in lock file")
            raise typer.Exit(1)
    ...
    # Mode 2: Direct image reference
        ...
        if "/" in target and "@" not in target:
            ...
            if lock_data:
                ...
                if not settings.api_key:
                    design.info("Not found in HUD registry (try setting HUD_API_KEY for private environments)")
                else:
        ...
        if not lock_data:
            ...
            if manifest:
                ...
                if verbose:
                    design.info(f"Retrieved manifest (type: {manifest.get('mediaType', 'unknown')})")
    ...
    if ":" in image_ref and "@" in image_ref:
        display_ref = image_ref.split("@")[0]
    else:
        display_ref = image_ref
    ...
    if lock_data and lock_data.get("source") != "docker-manifest":
        ...
        if "@sha256:" in image_ref:
            digest = image_ref.split("@sha256:")[-1][:12]
        else:
            digest = "latest"
    ...
    if ":" in image_ref and "@" in image_ref:
        simple_ref = image_ref.split("@")[0]
    else:
        simple_ref = image_ref
