            ...
            if config_path.exists():
                try:
                    ...
                    for registry_url, auth_info in auths.items():
                        if any(hub in registry_url for hub in ["docker.io", "index.docker.io", "registry-1.docker.io"]):
                            # Try to decode the auth to get username
...
def push_environment(
    ...
    if not image:
        ...
        if username:
            ...
            if "@" in local_image:
                base_image = local_image.split("@")[0]
            else:
            ...
            design.error("Not logged in to Docker Hub. Please specify --image or run 'docker login'")
            raise typer.Exit(1)
    elif tag:
    ...
    console.print(f"[green]✓[/green] Updated lock file with registry image")
    ...
    try:
        ...
        if len(registry_parts) >= 2:
            ...
            design.progress_message(f"Uploading metadata to HUD registry...")
            ...
            if response.status_code in [200, 201]:
                ...
                console.print(f"  Others can now pull with: [cyan]hud pull {name_with_tag}[/cyan]\n")
            else:
                design.warning(f"Could not upload to registry: {response.status_code}")
                if verbose:
                    design.info(f"Response: {response.text}")
                console.print(f"  Share [cyan]hud.lock.yaml[/cyan] manually\n")
        else:
            if verbose:
                design.info("Could not parse registry path for upload")
            console.print(f"  Share [cyan]hud.lock.yaml[/cyan] to let others reproduce your exact environment\n")
    ...
    console.print(f'    [cyan]{{"docker": {{"image": "{pushed_digest}", "command": "auto"}}}}[/cyan]\n')
    
    console.print("  Via HUD (recommended):")
    console.print(f'    [cyan]{{"hud": {{"registry": "{pushed_digest}"}}}}[/cyan]\n')
    
    console.print("Share environment:")
    console.print(f"  Share the updated [cyan]hud.lock.yaml[/cyan] for others to reproduce your exact environment")
