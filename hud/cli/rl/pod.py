...
async def create_and_connect_prime_pod(
    ...
    if team_id:
        ...
        design.debug(f"  Using personal account: default option [1]")
    
    design.debug(f"  Input sequence: provider={provider_choice}, disk={disk_size or 'default'}, image={image_choice}, team={team_choice if 'team_choice' in locals() else 'default'}")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if "invalid selection" in result.stdout.lower():
                ...
                design.info("")
                design.hint("The Prime CLI interface may have changed. Try running the command manually:")
                design.command_example(f"prime pods create --gpu-type {gpu_type} --gpu-count {gpu_count} --name {pod_name}")
            ...
            for i, (part, label) in enumerate(zip(input_parts, input_labels)):
                if part:
...
async def poll_pod_status(pod_id: str) -> str | None:
    ...
    # Create spinner
    spinner = Spinner("dots", text="Waiting for pod to become active (should take 5-20 min)...", style="gold")
