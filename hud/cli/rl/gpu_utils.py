...
def get_gpu_memory_info() -> dict[int, dict[str, Any]]:
    ...
    try:
        ...
        for gpu_idx in gpu_memory:
            cmd = ["nvidia-smi", "-i", str(gpu_idx), "--query-compute-apps=pid,used_memory", "--format=csv,noheader,nounits"]
            try:
                result = subprocess.run(cmd, capture_output=True, text=True, check=True)
                processes = []
                for line in result.stdout.strip().split("\n"):
    ...
    for gpu_idx in gpu_indices:
        ...
        if gpu_idx in memory_info:
            ...
            if mem["used_pct"] > 70:
                ...
                table.add_row(f"GPU {gpu_idx}", mem_str, "❌ Unhealthy", f"High memory usage{proc_info}")
                continue
...
def calculate_optimal_gpu_allocation(gpu_info: dict[str, Any], config) -> dict[str, Any]:
    ...
    return {
        ...
        "parallel_efficiency": min(1.0, num_groups / len(training_gpus))  # 1.0 = perfect load balance
    }


def adjust_config_for_ddp(config, num_gpus: int):
    ...
    from rich.console import Console
    console = Console()
    console.print(f"\n[cyan]📊 Adjusted batch_size to {config.training.batch_size} ({config.training.batch_size // group_size} groups)[/cyan]")
    console.print(f"[cyan]   Each of the {num_gpus} GPU(s) will process {config.training.batch_size // group_size // num_gpus} group(s) in parallel[/cyan]")
        ...
        if info["used_pct"] > memory_threshold:
            for proc in info.get("processes", []):
                ...
                try:
                    ...
                    killed_count += 1
                    console.print(f"[yellow]Terminating PID {pid} on GPU {gpu_idx} ({proc['memory_mb']/1024:.1f} GB)[/yellow]")
                except Exception as e:
