    ...
    try:
        ...
        if active_models or model is None:
            ...
            for m in active_models:
                ...
                choices.append({
                    "name": f"{status_emoji} {m.name} ({m.status})",
                    "value": m.name
                })
        ...
        if selected == "__new__":
                ...
                with hud_console.progress() as progress:
                    ...
                    while True:
                        ...
                        if model_info.status == "ready":
                            hud_console.success(f"vLLM server ready at http://rl.hud.so/v1/models/{model_name}/vllm")
                            break
            ...
            if model_info.status == "training":
                if hud_console.confirm(f"{model_name} is currently training. Stop current training?", default=False): # noqa: E501
