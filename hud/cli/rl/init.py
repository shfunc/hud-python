...
from .utils import get_image_from_lock, read_lock_file
...
async def generate_from_lock(image: str, tools: list[dict], output: Path | None, force: bool) -> None:
    ...
    design.info(f"Source: hud.lock.yaml")
    ...
    design.command_example(f"hud hf tasks.json --name my-tasks", "Create dataset")
...
async def generate_config(image: str, tools: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    interaction_tools = [
        t for t in tools 
        if t["name"] not in ["setup", "evaluate"]
    ]
