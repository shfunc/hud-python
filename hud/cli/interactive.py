...
class InteractiveMCPTester:
    ...
    async def select_tool(self) -> Optional[Any]:
    ...
    async def get_tool_arguments(self, tool: Any) -> Optional[dict[str, Any]]:
                ...
                if prop_type == 'boolean':
                    if is_required:
                        ...
                        choice = await questionary.select(
                            prompt,
                            choices=["true", "false", "skip (leave unset)"]
                        ).unsafe_ask_async()
                    ...
                    value_str = await questionary.text(
                        ...
                        validate=lambda text: True if not text and not is_required else (
                            text.replace("-", "").replace(".", "").isdigit() if prop_type == 'number' 
                            else text.replace("-", "").isdigit()
                        ) or f"Please enter a valid {prop_type}"
                    ).unsafe_ask_async()
