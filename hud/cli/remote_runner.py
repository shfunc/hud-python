...
from ..clients.utils.retry_transport import create_retry_httpx_client
...
def run_remote_stdio(
    ...
    async def run():
        ...
        if verbose:
            ...
            print(f"📦 Image: {headers.get('Mcp-Image', 'unknown')}", file=sys.stderr)
            print(f"🔑 Headers: {list(headers.keys())}", file=sys.stderr)
    ...
    try:
        ...
        if verbose:
            print("\n✅ Remote proxy stopped", file=sys.stderr)
        sys.exit(0)
    except Exception as e:
        if verbose:
