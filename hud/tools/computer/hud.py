...
from ..base import tool_result_to_content_blocks, ToolResult, ToolError
from ..executors.xdo import XDOExecutor
...
class HudComputerTool:
    ...
    def __init__(self, width: int = 1920, height: int = 1080, display_num: int | None = None):
        """
    ...
    async def __call__(
        ...
        """
        logger.info(f"HudComputerTool executing action: {action}")
        ...
        try:
            ...
            screenshot_actions = {
                "screenshot", "click", "type", "scroll", "move", "drag", 
                "press", "keydown", "keyup", "wait", "hold_key", 
                "mouse_down", "mouse_up"
            }
            ...
            raise McpError(ErrorData(code=INVALID_PARAMS, message=f"Invalid parameters for {action}: {str(e)}"))
    ...
    async def click(
        ...
        if pattern:
            ...
            if x is not None and y is not None:
                result = await self.executor.execute(f"mousemove {x} {y} click --repeat {click_count} --delay {delay} {button_num}", take_screenshot=False)
            else:
                result = await self.executor.execute(f"click --repeat {click_count} --delay {delay} {button_num}", take_screenshot=False)
        else:
            # Single click
            if x is not None and y is not None:
                result = await self.executor.execute(f"mousemove {x} {y} click {button_num}", take_screenshot=False)
            else:
    ...
    async def keydown(self, keys: list[str]) -> ToolResult:
        ...
        for key in keys:
            ...
            last_result = await self.executor.execute(f"keydown {escaped_key}", take_screenshot=False)
    ...
    async def move(
        ...
        if x is not None and y is not None:
            ...
            pos_result = await self.executor.execute("getmouselocation", take_screenshot=False)
    ...
    async def mouse_down(self, button: Literal["left", "right", "middle", "back", "forward"] = "left") -> ToolResult:
        """
    ...
    async def mouse_up(self, button: Literal["left", "right", "middle", "back", "forward"] = "left") -> ToolResult:
        """
