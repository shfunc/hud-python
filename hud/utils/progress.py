...
class StepProgressTracker:
    ...
    """
    def __init__(self, total_tasks: int, max_steps_per_task: int):
        if total_tasks <= 0:
    ...
    def increment_step(self, task_id: str):
        ...
        if not self._finished_tasks[task_id] and self._task_steps[task_id] < self.max_steps_per_task:
    ...
    def get_stats(self) -> Tuple[float, Optional[float]]:
        ...
        if rate_sec > 0:
            ...
            if remaining_steps > 0:
                eta = remaining_steps / rate_sec 
            else:
                eta = 0.0 # Already done or exceeded
    ...
    def display(self, bar_length=40) -> str:
